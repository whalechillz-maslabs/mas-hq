...
const correctSchedules = [
  ...
  { date: '2025-08-13', start: '10:00', end: '17:00', hours: 6, break_minutes: 60 }, // 수 (점심시간 제외)
  { date: '2025-08-14', start: '15:00', end: '17:00', hours: 2 }, // 목
  { date: '2025-08-18', start: '10:00', end: '17:00', hours: 6, break_minutes: 60 }, // 월 (점심시간 제외)
  { date: '2025-08-20', start: '10:00', end: '17:00', hours: 6, break_minutes: 60 }, // 수 (점심시간 제외)
  { date: '2025-08-22', start: '10:00', end: '17:00', hours: 6, break_minutes: 60 }  // 금 (점심시간 제외)
];

async function fixChoiAugustSchedulesCorrect() {
  try {
    console.log('🔧 최형호 8월 스케줄 정확히 수정 시작...');
    ...
    const schedulesToInsert = correctSchedules.map(schedule => ({
      ...
      break_minutes: schedule.break_minutes || 0, // 점심시간 제외 (60분)
    ...
    console.log('✅ 최형호 8월 스케줄 정확히 수정 완료!');
    console.log('📊 수정된 스케줄:');
    correctSchedules.forEach((schedule, index) => {
      const breakInfo = schedule.break_minutes ? ` (점심 ${schedule.break_minutes}분 제외)` : '';
      console.log(`${index + 1}. ${schedule.date} (${getDayOfWeek(schedule.date)}): ${schedule.start}-${schedule.end} (${schedule.hours}시간)${breakInfo}`);
    });
