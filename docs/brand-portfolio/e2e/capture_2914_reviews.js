/**
 * 챔피온 헤리티지 티셔츠 T2102 (2914) 리뷰 이미지 캡처 스크립트
 * 리뷰 이미지 최대 20개 가져오기
 */
...
// 챔피온 헤리티지 티셔츠 제품 정보
const championHeritageTee = {
  id: '2914',
  name: 'champion-heritage-tee-t2102',
  productUrl: 'https://www.marpple.com/kr/product/detail?bp_id=2914',
  reviewUrl: 'https://www.marpple.com/kr/product/detail?bp_id=2914#review',
  reviewFilenames: []
};
...
async function main() {
  ...
  try {
    console.log('🚀 챔피온 헤리티지 티셔츠 T2102 (2914) 리뷰 이미지 캡처 시작...\n');

    // 리뷰 이미지 가져오기
    console.log('📸 리뷰 이미지 가져오기...');
    console.log(`   URL: ${championHeritageTee.reviewUrl}`);
    
    await page.goto(championHeritageTee.reviewUrl, { 
    ...
    if (reviewImages.length > 0) {
      ...
      for (let i = 0; i < reviewImages.length; i++) {
        ...
        const tempPngPath = path.join(outputDir, `tees-2914-review-${i + 1}-temp.png`);
        const webpPath = path.join(outputDir, `tees-2914-review-${i + 1}.webp`);
        championHeritageTee.reviewFilenames.push(`tees-2914-review-${i + 1}.webp`);
        ...
        try {
          ...
          if (converted) {
            console.log(`✅ WEBP 변환 완료: tees-2914-review-${i + 1}.webp`);
            // 임시 PNG 파일 삭제
            ...
            // 변환 실패 시 PNG 파일명 변경
            const pngPath = path.join(outputDir, `tees-2914-review-${i + 1}.png`);
            fs.renameSync(tempPngPath, pngPath);
            console.log(`⚠️  WEBP 변환 실패, PNG로 저장됨: tees-2914-review-${i + 1}.png`);
          }
        } catch (error) {
  ...
  console.log('\n--- 챔피온 헤리티지 티셔츠 T2102 (2914) 리뷰 이미지 캡처 완료 ---');
  console.log(`리뷰 이미지: ${championHeritageTee.reviewFilenames.length}개`);
}
