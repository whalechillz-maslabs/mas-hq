...
// 아메리칸어패럴 2001 프리미엄 티셔츠 제품 정보
const aa2001PremiumTee = {
  id: '3115',
  name: 'aa2001-premium-tee',
  productUrl: 'https://www.marpple.com/kr/product/detail?bp_id=3115',
  reviewUrl: 'https://www.marpple.com/kr/product/detail?bp_id=3115#review',
  reviewFilenames: []
};
...
async function main() {
  ...
  try {
    console.log('🚀 아메리칸어패럴 2001 프리미엄 티셔츠 3115 리뷰 이미지 캡처 시작...\n');

    // 리뷰 이미지 가져오기
    console.log('📸 리뷰 이미지 가져오기...');
    console.log(`   URL: ${aa2001PremiumTee.reviewUrl}`);
    
    await page.goto(aa2001PremiumTee.reviewUrl, { 
    ...
    if (reviewImages.length > 0) {
      ...
      for (let i = 0; i < reviewImages.length; i++) {
        ...
        const tempPngPath = path.join(outputDir, `tees-3115-review-${i + 1}-temp.png`);
        const webpPath = path.join(outputDir, `tees-3115-review-${i + 1}.webp`);
        aa2001PremiumTee.reviewFilenames.push(`tees-3115-review-${i + 1}.webp`);
        ...
        try {
          ...
          if (converted) {
            console.log(`✅ WEBP 변환 완료: tees-3115-review-${i + 1}.webp`);
            // 임시 PNG 파일 삭제
            ...
            // 변환 실패 시 PNG 파일명 변경
            const pngPath = path.join(outputDir, `tees-3115-review-${i + 1}.png`);
            fs.renameSync(tempPngPath, pngPath);
            console.log(`⚠️  WEBP 변환 실패, PNG로 저장됨: tees-3115-review-${i + 1}.png`);
          }
        } catch (error) {
  ...
  console.log('\n--- 아메리칸어패럴 2001 프리미엄 티셔츠 3115 리뷰 이미지 캡처 완료 ---');
  console.log(`리뷰 이미지: ${aa2001PremiumTee.reviewFilenames.length}개`);
}
