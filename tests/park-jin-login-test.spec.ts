...
test.describe('박진(JIN) 계정 로그인 테스트', () => {
  test.beforeEach(async ({ page }) => {
    await page.goto('http://localhost:3000/login');
  });

  test('박진 계정 로그인 테스트 (전화번호 + 패스워드)', async ({ page }) => {
    console.log('🔍 박진 계정 로그인 테스트 시작 (전화번호 + 패스워드)');
    
    // 박진 계정으로 로그인
    await page.fill('input[type="tel"]', '010-9132-4337');
    await page.fill('input[type="password"]', '91324337'); // 전화번호 8자리
  ...
  test('박진 계정 로그인 테스트 (핀번호)', async ({ page }) => {
    ...
    if (currentUrl.includes('/dashboard')) {
      ...
      // 스크린샷 캡처
      await page.screenshot({ 
        path: 'park-jin-login-pin-success.png', 
        fullPage: true 
      });
    } else {
      console.log('❌ 박진 계정 로그인 실패 (핀번호)');
      
      // 스크린샷 캡처
      await page.screenshot({ 
        path: 'park-jin-login-pin-failed.png', 
        fullPage: true 
      });
    }
  });
