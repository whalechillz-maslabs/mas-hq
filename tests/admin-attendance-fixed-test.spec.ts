...
test('관리자 출근 관리 수정된 데이터 로딩 기능 테스트', async ({ page }) => {
  // 로그인
  ...
  console.log('=== 관리자 출근 관리 수정된 데이터 로딩 기능 테스트 시작 ===');
  ...
  await page.screenshot({ path: 'playwright-report/admin-attendance-fixed.png', fullPage: true });
  console.log('✅ 수정된 데이터 로딩 기능 테스트 스크린샷 저장');
  
  console.log('=== 관리자 출근 관리 수정된 데이터 로딩 기능 테스트 완료 ===');
});
