...
test.describe('출근 체크 시간 및 실시간 근무 시간 테스트', () => {
  test('김탁수 출근 체크 후 한국 시간 및 실시간 근무 시간 확인', async ({ page }) => {
    console.log('🚀 출근 체크 시간 및 실시간 근무 시간 테스트 시작...');
    ...
    if (await workTimeElement.isVisible()) {
      ...
      // 0.0이 아닌 값이 표시되는지 확인
      if (workTimeText && !workTimeText.includes('0.0')) {
        console.log('✅ 실시간 근무 시간이 정상적으로 계산됨');
      } else {
        console.log('❌ 실시간 근무 시간이 0.0으로 표시됨');
      }
    }
    ...
    // 7. 1분 대기 후 근무 시간 변화 확인
    console.log('\n⏳ 1분 대기 후 근무 시간 변화 확인...');
    await page.waitForTimeout(65000); // 65초 대기 (1분 + 5초 여유)
    ...
    if (await workTimeElementAfter.isVisible()) {
      ...
      if (workTimeTextAfter && !workTimeTextAfter.includes('0.0')) {
        console.log('✅ 실시간 근무 시간 업데이트가 정상적으로 작동함');
      } else {
        console.log('❌ 실시간 근무 시간 업데이트가 작동하지 않음');
      }
    }
    ...
    // 9. 스크린샷 저장
    await page.screenshot({ path: 'test-results/attendance-time-test.png' });
    console.log('📸 시간 테스트 결과 스크린샷 저장');
    
    console.log('\n🎯 시간 테스트 완료!');
    console.log('='.repeat(50));
    console.log('✅ 출근 체크 시간 확인 완료');
