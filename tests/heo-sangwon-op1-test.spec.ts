...
test.describe('허상원 계정 OP1 전체 워크플로우 테스트', () => {
  ...
  test('허상원 OP1 전체 워크플로우 테스트', async ({ page }) => {
    console.log('🔍 허상원 OP1 전체 워크플로우 테스트 시작');
    ...
    // 업무 정보 입력
    const testTaskTitle = `허상원 OP1 테스트 ${Date.now()}`;
    ...
    await modal.locator('textarea[name="notes"]').fill('허상원 OP1 테스트용 업무');
    await modal.locator('input[name="customer_name"]').fill('허상원 테스트 고객');
    ...
    if (taskExists) {
      ...
      if (refundButtonExists) {
        ...
        if (modalExists) {
          ...
          await refundModal.locator('textarea[name="notes"]').fill('허상원 OP1 환불 테스트');
    ...
    await page.screenshot({ 
      path: 'test-results/heo-sangwon-op1-workflow-test.png',
      fullPage: true 
    });
    
    console.log('🎉 허상원 OP1 전체 워크플로우 테스트 완료!');
  });
});
