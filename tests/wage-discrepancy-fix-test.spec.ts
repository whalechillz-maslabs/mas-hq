...
test.describe('급여 체계 불일치 해결 테스트', () => {
  test('김탁수 월급제 일관성 확인', async ({ page }) => {
    console.log('=== 김탁수 월급제 일관성 확인 테스트 시작 ===');
    ...
    if (monthlyCount > 0) {
      console.log('✅ 개인 출근 관리에서 김탁수가 월급제로 표시됩니다.');
    ...
    if (employeeSelectCount > 0) {
      ...
      if (options > 1) {
        ...
        console.log('시급 관리 직원 목록:', optionTexts.slice(1)); // 첫 번째 제외
    ...
    await page.screenshot({ 
      path: 'tests/screenshots/wage-discrepancy-fix-test.png',
      fullPage: true 
    });
    
    console.log('=== 김탁수 월급제 일관성 확인 테스트 완료 ===');
  });
  
  test('급여 체계 일관성 종합 확인', async ({ page }) => {
    console.log('=== 급여 체계 일관성 종합 확인 테스트 시작 ===');
    ...
    console.log('--- 시급 관리 페이지 직원 목록 확인 ---');
    ...
    // 4. 스크린샷 촬영
    await page.screenshot({ 
      path: 'tests/screenshots/wage-system-consistency-test.png',
      fullPage: true 
    });
    
    console.log('=== 급여 체계 일관성 종합 확인 테스트 완료 ===');
  });
});
