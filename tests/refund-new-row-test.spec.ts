...
test.describe('환불 기능 - 새로운 로우 생성 테스트', () => {
  ...
  test('OP1 업무 생성 -> 완료 -> 환불 처리 (새로운 로우 생성 확인)', async ({ page }) => {
    ...
    // 1. 허상원으로 로그인
    console.log('1️⃣ 허상원 계정으로 로그인');
    
    // 로그인 폼이 나타날 때까지 대기
    await page.waitForSelector('input[placeholder="전화번호를 입력하세요"]', { timeout: 30000 });
    
    await page.fill('input[placeholder="전화번호를 입력하세요"]', '010-8948-4501');
    await page.fill('input[placeholder="PIN을 입력하세요"]', '89484501');
