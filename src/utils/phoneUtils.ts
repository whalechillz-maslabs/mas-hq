...
export function formatEmergencyContact(contact?: {
  ...
  return `${contact.name} (${contact.relationship}) - ${formatPhoneNumber(contact.phone)}`;
}
...
export function isSamePhoneNumber(
  ...
  return removePhoneHyphens(phone1) === removePhoneHyphens(phone2);
}
...
export function normalizePhoneForSearch(phone: string | null | undefined): string {
  ...
  // 숫자만 추출하고 소문자로 변환
  return removePhoneHyphens(phone).toLowerCase();
}
 ...
 * @returns 매치 여부
 */
export function phoneNumberContains(
  phone: string | null | undefined,
  searchTerm: string
): boolean {
