...
SELECT 
    ...
    STRING_AGG(code || ': ' || name, ', ' ORDER BY points DESC) as type_list
...
-- 4. 포인트 타입별 분류 뷰
CREATE OR REPLACE VIEW operation_types_by_points AS
SELECT 
    code, 
    name, 
    category, 
...
-- 5. 활성 업무 유형만 조회하는 함수
CREATE OR REPLACE FUNCTION get_active_operation_types()
RETURNS TABLE (
    id UUID,
    code VARCHAR(20),
    name VARCHAR(100),
...
BEGIN
    ...
    ORDER BY ot.category, ot.points DESC;
END;
$$ LANGUAGE plpgsql;
