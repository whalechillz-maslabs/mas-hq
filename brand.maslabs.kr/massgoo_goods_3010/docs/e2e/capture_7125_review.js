/**
 * 베이직 야구모자 (대량형) 7125 리뷰 이미지 캡처 스크립트
 * 이미지 1개만 가져오기
 */
...
const bulkCap = {
  ...
  productUrl: 'https://www.marpple.com/kr/product/detail?bp_id=7125',
  reviewUrl: 'https://www.marpple.com/kr/product/detail?bp_id=7125#review',
  reviewFilename: 'caps-7125-beige-review-1.webp'
};
 ...
 * 리뷰 이미지 찾기 (1개만)
 */
async function findReviewImage(page) {
...
async function main() {
  ...
  try {
    console.log('🚀 베이직 야구모자 (대량형) 7125 리뷰 이미지 캡처 시작...\n');

    // 리뷰 이미지 가져오기
    console.log('📸 리뷰 이미지 가져오기...');
    console.log(`   URL: ${bulkCap.reviewUrl}`);
    
    await page.goto(bulkCap.reviewUrl, { 
    ...
    const reviewImage = await findReviewImage(page);
    
    if (reviewImage) {
      console.log(`   발견된 리뷰 이미지: 1개`);
      
      // 임시 PNG 파일로 다운로드
      const tempPngPath = path.join(outputDir, 'caps-7125-beige-review-1-temp.png');
      const webpPath = path.join(outputDir, bulkCap.reviewFilename);
      
      try {
        await downloadImage(reviewImage.src, tempPngPath);
        console.log(`✅ 리뷰 이미지 다운로드 완료 (PNG)`);
        
        // WEBP로 변환
        const converted = await convertToWebp(tempPngPath, webpPath);
        if (converted) {
          console.log(`✅ WEBP 변환 완료: ${bulkCap.reviewFilename}`);
          // 임시 PNG 파일 삭제
          fs.unlinkSync(tempPngPath);
        } else {
          // 변환 실패 시 PNG 파일명 변경
          fs.renameSync(tempPngPath, path.join(outputDir, 'caps-7125-beige-review-1.png'));
          console.log(`⚠️  WEBP 변환 실패, PNG로 저장됨`);
        }
      } catch (error) {
        console.log(`   리뷰 이미지 다운로드 실패: ${error.message}`);
      }
    } else {
  ...
  console.log('\n--- 베이직 야구모자 (대량형) 7125 리뷰 이미지 캡처 완료 ---');
}
...
module.exports = { findReviewImage };
