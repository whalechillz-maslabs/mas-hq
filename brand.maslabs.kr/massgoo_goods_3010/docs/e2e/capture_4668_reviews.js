...
// 제멋 에디 오버핏 티셔츠 제품 정보
const eddyOversizedTee = {
  id: '4668',
  name: 'eddy-oversized-tee',
  productUrl: 'https://www.marpple.com/kr/product/detail?bp_id=4668',
  reviewUrl: 'https://www.marpple.com/kr/product/detail?bp_id=4668#review',
  reviewFilenames: []
};
...
async function main() {
  ...
  try {
    console.log('🚀 제멋 에디 오버핏 티셔츠 4668 리뷰 이미지 캡처 시작...\n');

    // 리뷰 이미지 가져오기
    console.log('📸 리뷰 이미지 가져오기...');
    console.log(`   URL: ${eddyOversizedTee.reviewUrl}`);
    
    await page.goto(eddyOversizedTee.reviewUrl, { 
    ...
    if (reviewImages.length > 0) {
      ...
      for (let i = 0; i < reviewImages.length; i++) {
        ...
        const tempPngPath = path.join(outputDir, `tees-4668-review-${i + 1}-temp.png`);
        const webpPath = path.join(outputDir, `tees-4668-review-${i + 1}.webp`);
        eddyOversizedTee.reviewFilenames.push(`tees-4668-review-${i + 1}.webp`);
        ...
        try {
          ...
          if (converted) {
            console.log(`✅ WEBP 변환 완료: tees-4668-review-${i + 1}.webp`);
            // 임시 PNG 파일 삭제
            ...
            // 변환 실패 시 PNG 파일명 변경
            const pngPath = path.join(outputDir, `tees-4668-review-${i + 1}.png`);
            fs.renameSync(tempPngPath, pngPath);
            console.log(`⚠️  WEBP 변환 실패, PNG로 저장됨: tees-4668-review-${i + 1}.png`);
          }
        } catch (error) {
  ...
  console.log('\n--- 제멋 에디 오버핏 티셔츠 4668 리뷰 이미지 캡처 완료 ---');
  console.log(`리뷰 이미지: ${eddyOversizedTee.reviewFilenames.length}개`);
}
