...
// 가죽 클러치백 제품 정보
const leatherClutchBag = {
  id: '4934',
  name: 'leather-clutch-bag',
  productUrl: 'https://www.marpple.com/kr/product/detail?bp_id=4934',
  reviewUrl: 'https://www.marpple.com/kr/product/detail?bp_id=4934#review',
  reviewFilenames: []
};

// 이미지 저장 경로
const outputDir = path.join(__dirname, '../../images/pouches');
...
async function findReviewImages(page) {
    ...
    try {
      ...
      for (const element of elements) {
        ...
        if (src && 
          ...
          if (boundingBox && boundingBox.width >= 150 && boundingBox.height >= 150) {
  ...
  return reviewImages.slice(0, 21); // 최대 21개 (리뷰 21개)
}
...
async function main() {
  ...
  try {
    console.log('🚀 가죽 클러치백 4934 리뷰 이미지 캡처 시작...\n');

    // 리뷰 이미지 가져오기
    console.log('📸 리뷰 이미지 가져오기...');
    console.log(`   URL: ${leatherClutchBag.reviewUrl}`);
    
    await page.goto(leatherClutchBag.reviewUrl, { 
    ...
    for (let i = 0; i < 10; i++) {
      ...
      // "더보기" 버튼 클릭 시도
      try {
        const moreButton = await page.$('button:has-text("더보기"), .more-button, [class*="more"]');
        if (moreButton) {
          await moreButton.click();
          await page.waitForTimeout(2000);
        }
      } catch (e) {
        // 버튼이 없거나 클릭할 수 없으면 무시
      }
    }
    ...
    if (reviewImages.length > 0) {
      ...
      for (let i = 0; i < reviewImages.length; i++) {
        ...
        const tempPngPath = path.join(outputDir, `pouch-4934-review-${i + 1}-temp.png`);
        const webpPath = path.join(outputDir, `pouch-4934-review-${i + 1}.webp`);
        leatherClutchBag.reviewFilenames.push(`pouch-4934-review-${i + 1}.webp`);
        ...
        try {
          ...
          if (converted) {
            console.log(`✅ WEBP 변환 완료: pouch-4934-review-${i + 1}.webp`);
            // 임시 PNG 파일 삭제
            ...
            // 변환 실패 시 PNG 파일명 변경
            const pngPath = path.join(outputDir, `pouch-4934-review-${i + 1}.png`);
            fs.renameSync(tempPngPath, pngPath);
            console.log(`⚠️  WEBP 변환 실패, PNG로 저장됨: pouch-4934-review-${i + 1}.png`);
          }
        } catch (error) {
  ...
  console.log('\n--- 가죽 클러치백 4934 리뷰 이미지 캡처 완료 ---');
  console.log(`리뷰 이미지: ${leatherClutchBag.reviewFilenames.length}개`);
}
