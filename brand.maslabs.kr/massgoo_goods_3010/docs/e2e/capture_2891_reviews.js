...
const casualCap = {
  id: '2891',
  name: 'casual-ballcap',
  productUrl: 'https://www.marpple.com/kr/product/detail?bp_id=2891',
  reviewUrl: 'https://www.marpple.com/kr/product/detail?bp_id=2891#review',
  reviewFilenames: []
};
...
async function main() {
  ...
  try {
    console.log('🚀 캐주얼 볼캡 2891 리뷰 이미지 캡처 시작...\n');

    // 리뷰 이미지 가져오기
    console.log('📸 리뷰 이미지 가져오기...');
    console.log(`   URL: ${casualCap.reviewUrl}`);
    
    await page.goto(casualCap.reviewUrl, { 
    ...
    if (reviewImages.length > 0) {
      ...
      for (let i = 0; i < reviewImages.length; i++) {
        ...
        const tempPngPath = path.join(outputDir, `caps-2891-review-${i + 1}-temp.png`);
        const webpPath = path.join(outputDir, `caps-2891-review-${i + 1}.webp`);
        casualCap.reviewFilenames.push(`caps-2891-review-${i + 1}.webp`);
        ...
        try {
          ...
          if (converted) {
            console.log(`✅ WEBP 변환 완료: caps-2891-review-${i + 1}.webp`);
            // 임시 PNG 파일 삭제
            ...
            // 변환 실패 시 PNG 파일명 변경
            const pngPath = path.join(outputDir, `caps-2891-review-${i + 1}.png`);
            fs.renameSync(tempPngPath, pngPath);
            console.log(`⚠️  WEBP 변환 실패, PNG로 저장됨: caps-2891-review-${i + 1}.png`);
          }
        } catch (error) {
  ...
  console.log('\n--- 캐주얼 볼캡 2891 리뷰 이미지 캡처 완료 ---');
  console.log(`리뷰 이미지: ${casualCap.reviewFilenames.length}개`);
}
