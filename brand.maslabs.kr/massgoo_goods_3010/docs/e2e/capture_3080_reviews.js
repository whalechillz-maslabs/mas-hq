/**
 * 베이직 야구모자 (단품) 3080 리뷰 이미지 캡처 스크립트
 * 리뷰 이미지 최대 20개 가져오기
 */
...
const singleCap = {
  ...
  productUrl: 'https://www.marpple.com/kr/product/detail?bp_id=3080',
  reviewUrl: 'https://www.marpple.com/kr/product/detail?bp_id=3080#review',
  reviewFilenames: []
};
...
async function main() {
  ...
  try {
    console.log('🚀 베이직 야구모자 (단품) 3080 리뷰 이미지 캡처 시작...\n');

    // 리뷰 이미지 가져오기
    console.log('📸 리뷰 이미지 가져오기...');
    console.log(`   URL: ${singleCap.reviewUrl}`);
    
    await page.goto(singleCap.reviewUrl, { 
    ...
    if (reviewImages.length > 0) {
      ...
      for (let i = 0; i < reviewImages.length; i++) {
        ...
        const tempPngPath = path.join(outputDir, `caps-3080-review-${i + 1}-temp.png`);
        const webpPath = path.join(outputDir, `caps-3080-review-${i + 1}.webp`);
        singleCap.reviewFilenames.push(`caps-3080-review-${i + 1}.webp`);
        ...
        try {
          ...
          if (converted) {
            console.log(`✅ WEBP 변환 완료: caps-3080-review-${i + 1}.webp`);
            // 임시 PNG 파일 삭제
            ...
            // 변환 실패 시 PNG 파일명 변경
            const pngPath = path.join(outputDir, `caps-3080-review-${i + 1}.png`);
            fs.renameSync(tempPngPath, pngPath);
            console.log(`⚠️  WEBP 변환 실패, PNG로 저장됨: caps-3080-review-${i + 1}.png`);
          }
        } catch (error) {
  ...
  console.log('\n--- 베이직 야구모자 (단품) 3080 리뷰 이미지 캡처 완료 ---');
  console.log(`리뷰 이미지: ${singleCap.reviewFilenames.length}개`);
}
