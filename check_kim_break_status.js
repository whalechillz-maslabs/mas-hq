...
async function checkKimBreakStatus() {
  const today = new Date().toISOString().split('T')[0];
  console.log(`🔍 ${today} 김탁수 휴식 상태 확인...`);
  
  // 먼저 김탁수의 실제 employee_id 찾기
  console.log('\n👤 김탁수 employee_id 찾기:');
  ...
  if (employeeError) {
    console.error('❌ employees 조회 오류:', employeeError);
    return;
  }
  
  if (!employees || employees.length === 0) {
    console.log('❌ 김탁수 직원을 찾을 수 없습니다.');
    return;
  }
  
  const kimEmployee = employees[0];
  console.log(`✅ 김탁수 찾음: ${kimEmployee.name} (ID: ${kimEmployee.id}, Employee ID: ${kimEmployee.employee_id})`);
...
checkKimBreakStatus().catch(console.error);