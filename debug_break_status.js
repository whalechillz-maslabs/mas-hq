...
async function debugBreakStatus() {
  console.log('🔍 김탁수 휴식 상태 관리자 페이지 반영 문제 분석...\n');
  ...
  try {
    ...
    // 2. 9월 17일 schedules 데이터 확인
    console.log('\n2️⃣ 9월 17일 Schedules 데이터:');
    ...
    // 3. 9월 17일 attendance 데이터 확인
    console.log('\n3️⃣ 9월 17일 Attendance 데이터:');
    ...
    // schedules 데이터가 있는 경우
    if (schedulesData && schedulesData.length > 0) {
      schedulesData.forEach((schedule, index) => {
        console.log(`\n  Schedule ${index + 1} 기반 레코드:`);
        console.log(`    status: ${schedule.status}`);
        console.log(`    employee_note: ${schedule.employee_note}`);
        ...
        if (schedule.status === 'break') {
          simulatedStatus = 'break';
        } else if (schedule.employee_note && 
                   schedule.employee_note.includes('휴식 시작') && 
                   !schedule.employee_note.includes('휴식 후 복귀')) {
          simulatedStatus = 'break';
        } else if (!schedule.actual_start) {
          simulatedStatus = 'not_checked_in';
        } else if (!schedule.actual_end) {
